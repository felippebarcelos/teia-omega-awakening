#Requires -Version 7
# Benchmark_Harness_v9.ps1
# TEIA v0.10.0 — Compact Manifest (VER_MINOR=10)
# Corpus: D7_REAL_MANIFEST.json (mesmo de v7/v8 — apples-to-apples)
# 4 cenarios: A:LZMA(7z-mx9-.7z) | B:Zstd-19-raw | C:Zstd+dict(bucket) | D:TEIA-Selector(melhor opcode)
# Novidade v0.10: cmp.zstd/cmp.xz (compact manifest 43+N bytes vs ~150B JSON) → flipa tiny + LZMA_JANELA
# Probe sets: tiny→{cmp.zstd} | small→{cmp.zstd,dict-small} | medium→{dict-medium,zstd.raw} | large→{cmp.xz,xz.raw,zstd.raw}
# Saida: Relatorio_Comparativo_v9.0.0.md
...
$BENCH_DIR   = "D:\TEIA_CLAUDE_AWAKENING\_bench_v9"
$DICT_DIR    = "$BENCH_DIR\dicts"
$TMP_DIR     = "$BENCH_DIR\tmp"
$REPORT_PATH = "D:\TEIA_CLAUDE_AWAKENING\Relatorio_Comparativo_v9.0.0.md"
...
Write-Host "[V9] Importando motor v0.10.0 — Compact Manifest..."
...
if (-not (Test-Path $DICT_MEDIUM_PATH)) {
    ...
    Write-Host "[V9] dict-medium copiado: $($DICT_MEDIUM_SHA.Substring(0,16))..."
} else {
    Write-Host "[V9] dict-medium presente: $($DICT_MEDIUM_SHA.Substring(0,16))..."
}
$medBytes  = [IO.File]::ReadAllBytes($DICT_MEDIUM_PATH)
$medActSha = [TEIA.Native.TEIANativeV100]::SHA256Hex($medBytes)
if ($medActSha -ne $DICT_MEDIUM_SHA) { throw "dict-medium SHA mismatch: $medActSha" }
Write-Host "[V9] dict-medium SHA OK: $($DICT_MEDIUM_SHA.Substring(0,16))... | $($medBytes.Length)B"

# ── dict-small (mesmo corpus de treino dos harnesses anteriores) ─────────────────
Write-Host ""
Write-Host "[V9] === TREINANDO dict-small ==="
...
Write-Host "[V9] dict-small: $($DICT_SMALL_SHA.Substring(0,16))... | $($dictSmall.DictBytes)B | $($dictSmall.TrainMs)ms"
...
Write-Host ""
Write-Host "[V9] Corpus: $corpusN arquivos — $classif"
Write-Host "[V9] Compact Manifest: tiny→{cmp.zstd} | small→{cmp.zstd,dict-small} | medium→{dict-medium,zstd.raw} | large→{cmp.xz,xz.raw,zstd.raw}"
...
function Compress-ZstdDict([byte[]]$data, [string]$dictPath) {
    $sw  = [Diagnostics.Stopwatch]::StartNew()
    $cmp = [TEIA.Native.TEIANativeV100]::CompressWithZstd($data, $ZSTD, $dictPath, 19)
    $sw.Stop()
    return [pscustomobject]@{ Bytes=$cmp.Length; Ms=$sw.ElapsedMilliseconds }
}
...
foreach ($bkt in $bucketOrder) {
    ...
    $bucketStats[$bkt] = [pscustomobject]@{
        ...
        Op_ZstdRaw=$nZ; Op_Dict=$nD; Op_XzRaw=$nXR; Op_CmpZstd=$nCZ; Op_CmpXz=$nCX
    }
    ...
    Write-Host ("          Perdas: LJ={0} DP={1} OC={2} | Opcodes: cmp.zstd={3} cmp.xz={4} dict={5} zstd.raw={6} xz.raw={7}" -f `
        $lLJ, $lDP, $lOC, $nCZ, $nCX, $nD, $nZ, $nXR)
}
...
[void]$sb.AppendLine("# Relatorio Comparativo v9.0.0 — TEIA Compact Manifest ($totalAll arquivos)")
[void]$sb.AppendLine("")
...
[void]$sb.AppendLine("> **Corpus**: D7_REAL_MANIFEST.json | $totalAll arquivos | zero overlap D4 | seed=42 (mesmo que v7/v8)")
[void]$sb.AppendLine("> **Classificacao**: $classif")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("---")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("## Compact Manifest v0.10 — Estrategia de Probe por Bucket")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("| Bucket | Opcodes Probados | Selecao |")
[void]$sb.AppendLine("|--------|-----------------|---------|")
[void]$sb.AppendLine("| tiny (<5KB) | cmp.zstd | cmp.zstd (overhead 72B vs 166B JSON) |")
[void]$sb.AppendLine("| small (5-100KB) | cmp.zstd + dict.zstd_shared(dict-small) | min(D) |")
[void]$sb.AppendLine("| medium (100-500KB) | dict.zstd_shared(dict-medium) + zstd.raw | min(D) |")
[void]$sb.AppendLine("| large (>=500KB) | cmp.xz + xz.raw + zstd.raw | min(D) |")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("**Compact manifest (VER_MINOR=10)**: `format_byte(1) + algo_byte(1) + orig_size(8) + sha256_raw(32) + name_len(1) + name(N)` = 43+N bytes.")
[void]$sb.AppendLine("Overhead tiny: ~72B (vs 166B JSON → -94B). Overhead large xz: ~66B (vs 160B JSON → -94B).")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("**Predicao matematica**: 20 tiny flip (A-B >= 139B >> 72B limiar), R96/R100 flip (cmp.xz overhead -94B resolve margem 73B/70B).")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("---")
[void]$sb.AppendLine("")
...
[void]$sb.AppendLine("")
[void]$sb.AppendLine("| Bucket | N | Wins | A% | B% | D% | Ovhd | LJ | DP | OC | C.zst | C.xz | Dict | Z.raw | X.raw | SHA |")
[void]$sb.AppendLine("|--------|---|------|----|----|----|------|----|----|-----|-------|------|------|-------|-------|-----|")
...
foreach ($bkt in $bucketOrder) {
    ...
    [void]$sb.AppendLine("| $($s.Bucket) | $($s.Count) | $($s.Wins)/$($s.Count) | $($s.AvgA)% | $($s.AvgB)% | $($s.AvgD)% | ~$($s.AvgOvhd)B | $($s.L_LJ) | $($s.L_DP) | $($s.L_OC) | $($s.Op_CmpZstd) | $($s.Op_CmpXz) | $($s.Op_Dict) | $($s.Op_ZstdRaw) | $($s.Op_XzRaw) | $shaStr3 |")
}
...
$glXR = @($results | Where-Object { $_.Op -eq "xz.raw" }).Count
$glCZ = @($results | Where-Object { $_.Op -eq "cmp.zstd" }).Count
$glCX = @($results | Where-Object { $_.Op -eq "cmp.xz" }).Count

[void]$sb.AppendLine("")
[void]$sb.AppendLine("---")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("## Diagnóstico de Perdas (v8 vs v9)")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("| Categoria | v8 (82/105) | v9 |")
[void]$sb.AppendLine("|-----------|------------|-----|")
...
if ($lossFiles.Count -eq 0) {
    [void]$sb.AppendLine("**Nenhuma perda. 105/105 TEIA ganha.**")
} else {
...
[void]$sb.AppendLine("---")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("## Distribuição de Opcodes (Selector Engine v0.10)")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("| Opcode | Manifest | Total selecionado | % arquivos |")
[void]$sb.AppendLine("|--------|----------|------------------|-----------|")
[void]$sb.AppendLine("| cmp.zstd | VER_MINOR=10 (compact) | $glCZ | $([math]::Round($glCZ/$totalAll*100,1))% |")
[void]$sb.AppendLine("| cmp.xz | VER_MINOR=10 (compact) | $glCX | $([math]::Round($glCX/$totalAll*100,1))% |")
[void]$sb.AppendLine("| dict.zstd_shared | VER_MINOR=9 (JSON) | $glD | $([math]::Round($glD/$totalAll*100,1))% |")
[void]$sb.AppendLine("| zstd.raw | VER_MINOR=9 (JSON) | $glZ | $([math]::Round($glZ/$totalAll*100,1))% |")
[void]$sb.AppendLine("| xz.raw | VER_MINOR=9 (JSON) | $glXR | $([math]::Round($glXR/$totalAll*100,1))% |")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("---")
[void]$sb.AppendLine("")
...
$savings = if ($winRows.Count -gt 0) { ($winRows | Measure-Object Delta_DA -Sum).Sum } else { 0 }
[void]$sb.AppendLine("")
...
[void]$sb.AppendLine("---")
[void]$sb.AppendLine("")
[void]$sb.AppendLine("*v8→v9 delta: TEIA $totalWins/105 (antes 82/105). Economia: ${savings}B (antes 103,418B).*")
...
Write-Host ""
Write-Host "[V9] COMPLETO — $REPORT_PATH"
Write-Host "[V9] Relatorio SHA-256: $writeSha"
Write-Host "[V9] $totalWins/$totalAll TEIA ganha | Savings: ${totalSavings}B | $totalErrors erros"
