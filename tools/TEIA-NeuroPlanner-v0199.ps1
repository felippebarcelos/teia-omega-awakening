<#
.SYNOPSIS
    TEIA-NeuroPlanner-v0199.ps1 — Motor Seletor Determinístico (Rule-First + HR5)

.DESCRIPTION
    Arquitetura v0.19.9 — árvore de decisão "Regra Primeiro, IA Depois":
      ...
      1. Hard Rules (determinísticas, sem LLM):
           ...
           size_bytes < 8192          → cmp.brotli (HR5 — Brotli supera LZMA em arquivos pequenos)
...
.EXAMPLE
    .\TEIA-NeuroPlanner-v0199.ps1 -Files "D:\TEIA_CORE\benchmarks\synth_planner\*.bin" -DryRun
    Get-ChildItem D:\TEIA_USER\Inbox -File | .\TEIA-NeuroPlanner-v0199.ps1
#>
[CmdletBinding()]
param(
...
function Get-HardRuleDecision {
    ...
    # Regra 5: arquivo pequeno (< 8 KB) → cmp.brotli
    # Brotli tem dicionário estático que supera a janela deslizante do LZMA em arquivos pequenos.
    # Evidência empírica v0.19.6: .teia_map.json (2.5KB) Brotli=23.1% vs LZMA=28.1%;
    # TEIA-Core-v0.4.0.ps1 (6.3KB) Brotli=35.7% vs LZMA=36.3%.
    if ([long]$Metrics.size_bytes -lt 8192) {
        return [ordered]@{
            source   = 'hard_rule'
            rule_id  = 'HR5'
            rule     = "size_bytes=$($Metrics.size_bytes) < 8192 (arquivo pequeno — Brotli supera LZMA por dicionário estático)"
            strategy = 'cmp.brotli'
        }
    }
    ...
    return [ordered]@{
        source   = 'llm_needed'
        rule_id  = $null
        rule     = 'entropy∈[2,7) sem período, size≥8KB — LLM decide lzma vs brotli'
        strategy = $null
    }
}
...
foreach ($filePath in $Files) {
    ...
    $verdictColor = if ($verdict -eq 'PROCEDURAL_CANDIDATE') { 'Green' }
                    elseif ($verdict -eq 'BROTLI_PREFERRED')  { 'Cyan' }
...
Write-Host ' RELATÓRIO FINAL — TEIA-NeuroPlanner-v0199' -ForegroundColor Cyan
Write-Host $('=' * 70)
...
[object[]]$hrSmall = @($allProfiles | Where-Object { $_.entropy -ge 2.0 -and $_.entropy -lt 7.0 -and $_.period_bytes -eq 0 -and $_.unique_bytes -gt 1 -and [long]$_.size_bytes -lt 8192 })
[object[]]$medium  = @($allProfiles | Where-Object { $_.entropy -ge 2.0 -and $_.entropy -lt 7.0 -and $_.period_bytes -eq 0 -and $_.unique_bytes -gt 1 -and [long]$_.size_bytes -ge 8192 })
...
if ($hrSmall.Count -gt 0) {
    Write-Host "  HR5_BROTLI ($($hrSmall.Count)) — Hard Rule HR5 (size < 8 KB → cmp.brotli):" -ForegroundColor Cyan
    foreach ($p in $hrSmall) { Write-Host "    ✓ $($p.name) [HR5 size=$($p.size_bytes)B e=$($p.entropy) → cmp.brotli]" -ForegroundColor Cyan }
}
