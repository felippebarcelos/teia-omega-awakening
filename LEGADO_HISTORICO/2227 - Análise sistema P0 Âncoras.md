# 2227 - Análise sistema P0 Âncoras
...
Successfully created text document 'Portal P0 — Síntese Canônica Do Sistema Âncora (teia.seed.anchor.v1)' which will be referenced in all future messages with the unique identifier textdoc_id: '68ffd5115cf88191944d20d36f4f55e2'
...
Pronto! Coloquei a **síntese canônica do Portal P0/Âncora** na lateral como um `.md` limpo, seguindo exatamente a estrutura que você pediu. Se quiser, eu também exporto como arquivo `.md` baixável.

### Notas e fontes (trechos-chave)
- **Estrutura da seed-âncora (version/target/length/sha256/frag/anchors[3])** e offsets (início, meio centrado, fim) — base dos scripts de geração e restauração. fileciteturn1file4  
- **Injeção WebCrypto no dashboard** para gerar `.anchor.seed.json` sem payload e mostrar KPIs (original vs seed). fileciteturn1file11  
- **Restauração por índice/scan**: BFS com pré-filtro por tamanho/nome + comparação das 3 âncoras e, só então, SHA integral; cópia verificada. fileciteturn1file13  
- **Prova P0 automática (relatório JSON)** com `ratio_seed_vs_orig` e `ratio_total_vs_orig` (core scripts + seed / original). fileciteturn1file16  
- **Métrica emblemática do P0** (core+seed/original ≈ 0,18%) e expectativa “SHA confere”. fileciteturn1file7 fileciteturn1file5

Quer que eu gere um **.md para download** ou já **congele uma versão “v1.0 – Portal P0”** para uso futuro?
