# 2074 - Otimização de planejamento TEIA
...
como podemos adaptar o transgredido no pc ao iphone pelo app pyto? "vou direto ao ponto — em cima do que já rodamos no iPhone com o Pyto.  O que já existe no Pyto (hoje) 	•	Árvore de trabalho preparada ~/Documents/Seeds/ com subpastas seeds, inbox, out, dict. (vimos listagens com “Found seeds: 3”, e um seed tipo IMG_8676.sid.json, modo manifest-only, apontando para o original em inbox e exibindo validação de chunks com SHA-256.) 	•	Seed “manifest-only” Ao colocar um arquivo em inbox, o fluxo atual consegue: 	1.	calcular tamanho total, 	2.	segmentar em chunks (p.ex. 120 para o MOV do exemplo), 	3.	registrar SHA-256 por chunk e hash global, 	4.	salvar um .sid.json em seeds com metadados + índice de chunks, 	5.	rodar um validador que re-lê o arquivo original e confirma os hashes (“validated 32… 64… 96…”). 	•	Validação ≠ Restauração O que está pronto é a verificação de integridade (com o arquivo original ainda presente). O que não está pronto é o gerador procedural que, dado núcleo + seed, emite o binário idêntico sem depender do original. 	•	Ambiente Pyto ajustado Permissões de pasta (iCloud + “Meu iPhone”), criação de arquivos de teste e execução local. Observação prática: o prompt “python3 launcher.py” não vale no shell embutido do Pyto — a execução é via o Run do Pyto (ou python), não python3 no estilo desktop.  ⸻  Workflow atual (núcleo procedural + seed) 	1.	Ingestão: mover arquivo-fonte para ~/Documents/Seeds/inbox/. 	2.	Indexação: calcular chunk map + SHA-256 e gerar *.sid.json em ~/Documents/Seeds/seeds/ (modo manifest-only). 	3.	Validação: ler o arquivo em inbox e confirmar todos os hashes de chunks (barra de progresso “validated …”). 	4.	(Opcional futuro) Exportar/empacotar seed + métricas. 	5.	Falta: etapa “Restore” puramente procedural (seed + núcleo ⇒ bytes idênticos ao original, sem tocar o arquivo em inbox).  Em termos simples: hoje o seed descreve como o arquivo é, não como sintetizá-lo a partir de uma base universal.  ⸻  Por que isso ainda não é “autonomia real procedural”  Para atingir “recuperar um arquivo maior que a soma (seed + núcleo)” com restauração idempotente (mesmo SHA-256), faltam pilares: 	1.	Conteúdo atômico de base (DICT) real O diretório dict está preparado, mas ainda não contém um dicionário universal de átomos binários (ou operadores) compartilháveis entre arquivos. Sem esse repositório, o seed precisa carregar informação demais — por isso permanece “manifest-only”. 	2.	Codec reversível (gerador procedural) Não há, ainda, um codec que, a partir de opcodes e parâmetros referenciados no seed, gere o fluxo de bytes do arquivo. O manifest atual indexa o original; ele não contém um programa de geração com baixo custo de descrição. 	3.	Modelo de compressão universal (além de hashes) Para que |seed|+|núcleo| < |arquivo| e ainda assim restaurar bit-a-bit, precisamos de: 	•	codebooks (dicionários) compartilhados, 	•	referências aos trechos do codebook (não ao arquivo original), 	•	possível aprendizado/extração de padrões cross-arquivo (sem “treino online pesado”, mas com dicionários estáveis e versionados). 	4.	Protocolo de integridade & correção de erro Faltam FEC/ECC (p.ex., Reed-Solomon/Luby) e um endereçamento de conteúdo (CID) que permita recompor átomos do dicionário sem ambiguidade e com verificação determinística (hoje temos SHA-256, mas só sobre o original). 	5.	Orquestração no Pyto (UX de execução) Está ausente o comando de restauração que: seed(.sid.json) + nucleo(DICT+codec) ⇒ out/arquivo_restaurado e fecha o ciclo com hash match. O shell do Pyto requer um launcher próprio (não python3).  ⸻  O que significa, na prática, “autonomia real procedural”  A meta é cumprir, para qualquer arquivo alvo A: 	•	Entrada: 	•	Núcleo estático (codecs + dicionários) de tamanho finito e reutilizável; 	•	Seed mínimo e específico daquele A (referências + parâmetros + checks). 	•	Condição: |núcleo| + |seed_A|  <<  |A| (estritamente menor, com folga prática). 	•	Saída: Um gerador que não acessa A nem suas cópias, mas produz A’ tal que SHA256(A’) == SHA256(A).  Hoje temos: estrutura de pastas + manifest + validação via original. Falta: dicionário vivo, codec reversível, gerador procedural, restore sem original.  ⸻  Como fechar a lacuna (resumo operacional, sem código) 	1.	Definir o “Núcleo v0”: 	•	DICT-v0: um conjunto inicial de átomos binários e operadores (run-length, LZ-like, wavelet/transform para mídia, etc.) versionados. 	•	CODEC-v0: gramática/VM mínima para executar “programas de geração”. 	2.	Evoluir o seed de manifest-only para programático: 	•	Em vez de listar “chunks & hashes do original”, o seed passa a referenciar átomos do DICT e instruções do CODEC (com poucos parâmetros). 	•	Mantém verificadores (SHA-256 por bloco lógico + global). 	3.	Implementar RESTORE no Pyto: 	•	“Seed runner” que carrega núcleo + seed ⇒ emite bytes em out e fecha com verificação. 	4.	Métricas de autonomia (no iPhone): 	•	ratio = (|núcleo| + |seed|) / |arquivo| 	•	time_restore, energy_proxy, RAM_peak, SHA_match (True/False). 	•	Tabelar por tipo de arquivo (vídeo, áudio, texto, binário arbitrário).  ⸻  Em uma frase  No iPhone, o que já construímos no Pyto cria e valida seeds manifest-only (com o arquivo presente), mas ainda não temos o dicionário universal + codec reversível + gerador procedural para restaurar o arquivo apenas com seed + núcleo — por isso ainda não alcança a autonomia real procedural que comprime a informação descritiva abaixo do tamanho do arquivo e reconstrói o original de forma idempotente."

---

### ASSISTENTE

bora portar o que já “quebrou inércia” no PC para o iPhone (Pyto) de forma simples e **seed-first**. A ideia: um módulo único em Python, sem dependências externas, que replica as rotas essenciais (seed-anchor → index → locate/restore → catálogo/dry-run → dashboard), tudo confinado à pasta do Pyto.

abaixo vai um **kit Pyto v0.5** pronto pra colar no app:

---

# 1) Onde ficam as coisas no iPhone (Pyto)

- Base: `~/Documents/Seeds/`
  - `inbox/` → coloque aqui os arquivos de origem (ex.: vídeos/fotos).
  - `seeds/` → onde as seeds serão salvas (`*.anchor.sid.json`).
  - `out/` → onde os restaurados vão parar.
  - `dict/` → reservado para o futuro **DICT v0** (procedural real).
  - `reports/` e `dashboard/` → relatórios e HTML.

O Pyto só enxerga o sandbox dele; então focamos nessa árvore.

---

# 2) Cole este arquivo no Pyto como `teia_ios.py`

> Novo Script → cole tudo → salve como **`teia_ios.py`**

```python
# teia_ios.py — TEIA seed-first (Pyto, iOS)
# Núcleo mínimo: seed âncora, index, locate/restore, catálogo, dry-run, dashboard, prova P0.
# Stdlib only.

from __future__ import annotations
import os, json, hashlib, time, webbrowser
...
# Diretórios / paths
# =========================
BASE = Path.home() / "Documents" / "Seeds"
DIR_INBOX = BASE / "inbox"
DIR_SEEDS = BASE / "seeds"
DIR_OUT   = BASE / "out"
DIR_DICT  = BASE / "dict"
DIR_REP   = BASE / "reports"
DIR_DASH  = BASE / "dashboard"

ANCHOR_INDEX = DIR_SEEDS / "anchor_index.json"

FRAG = 65536  # 64 KiB

def _ensure_dirs():
    for d in (BASE, DIR_INBOX, DIR_SEEDS, DIR_OUT, DIR_DICT, DIR_REP, DIR_DASH):
...
def _read_range_sha256(p: Path, offset: int, length: int) -> str:
    with p.open("rb") as f:
        f.seek(max(0, offset))
        data = f.read(max(0, length))
    return _sha256_bytes(data)

def _anchors_for(p: Path, frag: int = FRAG) -> Tuple[str, str, str]:
    size = p.stat().st_size
    off_first = 0
    off_mid   = max(0, size//2 - frag//2)
    off_last  = max(0, size - frag)
    a = _read_range_sha256(p, off_first, frag)
    b = _read_range_sha256(p, off_mid,   frag)
    c = _read_range_sha256(p, off_last,  frag)
    return a, b, c

def _seed_path_for(target_name: str) -> Path:
    # usa sufixo .anchor.sid.json (compatível com teu histórico no iPhone)
    safe = target_name
    return DIR_SEEDS / f"{safe}.anchor.sid.json"

# =========================
# Seed âncora (manifest-only)
# =========================
def seed_anchor(input_path: str | Path, out_seed: Optional[str | Path] = None, frag: int = FRAG) -> Path:
    """Gera seed-âncora (first/mid/last) + SHA256 global. Não copia payload."""
    _ensure_dirs()
    src = Path(input_path).expanduser().resolve()
    if not src.exists() or not src.is_file():
        raise FileNotFoundError(f"Arquivo não encontrado: {src}")
    size = src.stat().st_size
    sha_full = _sha256_file(src)
    a, b, c = _anchors_for(src, frag)

    seed = {
        "version": "anchor.seed.v1",
        "target": src.name,
        "length": int(size),
        "sha256": sha_full,
        "frag": int(frag),
    ...
    if out_seed is None:
        out_seed = _seed_path_for(src.name)
    out_seed = Path(out_seed)
...
# =========================
# Anchor-Index (da pasta inbox/)
# =========================
def build_anchor_index(root: Path | str = DIR_INBOX, out_path: Path | str = ANCHOR_INDEX, frag: int = FRAG) -> Path:
    """Indexa APENAS a pasta root (por padrão inbox/) para não varrer iPhone inteiro."""
    _ensure_dirs()
...
# =========================
# Locate / Restore (via index)
# =========================
def _load_json(p: Path) -> Any:
    return json.loads(Path(p).read_text(encoding="utf-8"))

def locate_in_index(seed_path: str | Path, index_path: str | Path = ANCHOR_INDEX, top_n: int = 5) -> List[Dict[str, Any]]:
    seed = _load_json(seed_path)
    idx  = _load_json(index_path)
    tgt_len = int(seed["length"])
    tgt_anc = seed.get("anchors", [])
    res = []
    for it in idx.get("items", []):
        if int(it.get("length", -1)) != tgt_len:
            continue
        a = it.get("anchors", [])
        match = sum(1 for i in range(min(len(a), len(tgt_anc))) if a[i] == tgt_anc[i])
        if match > 0:
            res.append({"path": it["path"], "match": match, "length": it["length"], "frag": it["frag"]})
    res.sort(key=lambda r: (-r["match"], r["path"]))
    return res[:top_n]

def restore_from_index(seed_path: str | Path, out_path: str | Path, index_path: str | Path = ANCHOR_INDEX) -> Path:
    """Copia o original do inbox (encontrado via anchors) para out/, valida SHA."""
    _ensure_dirs()
    hits = locate_in_index(seed_path, index_path=index_path, top_n=1)
    if not hits:
        raise RuntimeError("Nenhuma fonte encontrada no índice para esta seed.")
    src = Path(hits[0]["path"])
    outp = Path(out_path).expanduser().resolve()
    outp.parent.mkdir(parents=True, exist_ok=True)
    # copia streaming
    with src.open("rb") as fi, outp.open("wb") as fo:
        for chunk in iter(lambda: fi.read(1024*1024), b""):
            fo.write(chunk)
    # valida
    seed = _load_json(seed_path)
    sha_out = _sha256_file(outp)
    if sha_out.lower() != seed["sha256"].lower():
        raise RuntimeError(f"SHA mismatch: out={sha_out} seed={seed['sha256']}")
    return outp

# =========================
# Catálogo e Dry-Run
# =========================
def seed_catalog(seed_dir: Path | str = DIR_SEEDS, out_path: Path | str = (DIR_REP / "seed_catalog.json")) -> Path:
    seed_dir = Path(seed_dir)
    items, seed_bytes_total, orig_bytes_total = [], 0, 0
    for p in sorted(seed_dir.rglob("*.sid.json")) + sorted(seed_dir.rglob("*.seed.json")):
        try:
            j = _load_json(p)
            kind = "anchor" if "anchors" in j else ("payload" if "payload" in j else "unknown")
            sp = p.stat().st_size
            items.append({
                "seed_path": str(p),
                "seed_bytes": sp,
                "target": j.get("target"),
                "length": int(j.get("length", 0)),
                "sha256": j.get("sha256"),
                "kind": kind,
            })
            seed_bytes_total += sp
            orig_bytes_total += int(j.get("length", 0))
        except Exception:
            pass
    meta = {
        "version": "seed-catalog.v1",
        ...
        "seed_bytes_total": seed_bytes_total,
        "orig_bytes_total": orig_bytes_total,
        "ratio_seed_to_orig": (seed_bytes_total / orig_bytes_total) if orig_bytes_total else 0.0,
        "created": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    }
    outp = Path(out_path)
    outp.parent.mkdir(parents=True, exist_ok=True)
    (DIR_REP / "seed_catalog_meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    Path(outp).write_text(json.dumps({"meta": meta, "items": items}, ensure_ascii=False), encoding="utf-8")
    return outp

def seed_dryrun(seed_dir: Path | str = DIR_SEEDS, index_path: Path | str = ANCHOR_INDEX, out_path: Path | str = (DIR_REP / "dryrun_evict_report.json")) -> Path:
    has_index = Path(index_path).exists()
    ok, maybe, unsafe = [], [], []
    for p in sorted(Path(seed_dir).rglob("*.sid.json")) + sorted(Path(seed_dir).rglob("*.seed.json")):
        try:
            s = _load_json(p)
            kind = "anchor" if "anchors" in s else ("payload" if "payload" in s else "unknown")
            length = int(s.get("length", 0))
            if kind == "payload":
                ok.append({"seed": str(p), "kind": kind, "target": s.get("target"), "length": length, "reason": "payload"})
            elif kind == "anchor":
                if not has_index:
                    maybe.append({"seed": str(p), "kind": kind, "target": s.get("target"), "length": length, "reason": "anchor-no-index"})
                else:
                    hits = locate_in_index(p, index_path=index_path, top_n=1)
                    if hits:
                        ok.append({"seed": str(p), "kind": kind, "target": s.get("target"), "length": length, "reason": "found-in-index"})
                    else:
                        unsafe.append({"seed": str(p), "kind": kind, "target": s.get("target"), "length": length, "reason": "not-in-index"})
            else:
                unsafe.append({"seed": str(p), "kind": "unknown", "target": s.get("target"), "length": length, "reason": "unknown-seed"})
        except Exception as e:
            unsafe.append({"seed": str(p), "kind": "invalid", "target": "?", "length": 0, "reason": f"parse-error {e}"})
    def _sum(lst): return int(sum(int(x.get("length",0)) for x in lst))
    report = {
        "version": "seed-dryrun.v1",
        "seed_dir": str(seed_dir),
        "index": str(index_path) if has_index else None,
        "counts": {"ok": len(ok), "maybe": len(maybe), "unsafe": len(unsafe)},
        "bytes": {"ok": _sum(ok), "maybe": _sum(maybe), "unsafe": _sum(unsafe)},
        "created": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "ok": ok, "maybe": maybe, "unsafe": unsafe
    }
    outp = Path(out_path)
...
def dashboard():
    ...
    # tenta carregar reports
    for k, p in {
        "catalog": DIR_REP / "seed_catalog.json",
        "dryrun":  DIR_REP / "dryrun_evict_report.json",
        "index":   ANCHOR_INDEX
    }.items():
        if p.exists():
            try:
                status[k] = json.loads(p.read_text(encoding="utf-8"))
            except Exception as e:
                status.setdefault("notes", []).append(f"{k} inválido: {e}")
        else:
            status.setdefault("notes", []).append(f"{k} ausente: {p}")
    (DIR_REP / "supra_status.json").write_text(json.dumps(status, ensure_ascii=False, indent=2), encoding="utf-8")
    # HTML simples
    html = f"""<!doctype html>
<html><head><meta charset="utf-8"><title>TEIA • iOS Dashboard</title>
<style>body{{font-family:-apple-system,system-ui,Segoe UI,Roboto,Arial,sans-serif;padding:16px}}
.card{{border:1px solid #e5e7eb;border-radius:12px;padding:12px;margin:10px 0}}</style></head>
<body>
<h1>TEIA • iOS Dashboard</h1>
<div class="card"><pre id="status"></pre></div>
<div class="card"><h3>Catalog</h3><pre id="catalog"></pre></div>
<div class="card"><h3>Dry-run</h3><pre id="dryrun"></pre></div>
<script>
fetch("../reports/supra_status.json").then(r=>r.json()).then(j=>{{
  document.getElementById("status").textContent = JSON.stringify({{timestamp:j.timestamp,root:j.root,notes:j.notes}},null,2);
  document.getElementById("catalog").textContent = JSON.stringify(j.catalog??"(sem)",null,2);
  document.getElementById("dryrun").textContent  = JSON.stringify(j.dryrun??"(sem)",null,2);
}});
</script>
</body></html>"""
    dash = DIR_DASH / "index.html"
...
def proof_p0(sample_name: str = None) -> Dict[str, Any]:
    """Prova P0: seed âncora + index + restore + SHA."""
    _ensure_dirs()
    # escolhe um arquivo do inbox se não informado
    if sample_name is None:
        # pega o primeiro arquivo do inbox
        cand = next((p for p in sorted(DIR_INBOX.iterdir()) if p.is_file()), None)
        if cand is None:
            raise RuntimeError(f"Coloque um arquivo em: {DIR_INBOX}")
        src = cand
    else:
        src = (DIR_INBOX / sample_name) if not Path(sample_name).is_file() else Path(sample_name)
    seed = seed_anchor(src)
    index = build_anchor_index(DIR_INBOX, ANCHOR_INDEX)
    outp = DIR_OUT / f"{src.stem}.anchor.restored{src.suffix}"
    outp = restore_from_index(seed, outp, index)
    ok = (_sha256_file(src) == _sha256_file(outp))
    rep = {
        "file": str(src),
        "seed": str(seed),
        "restored": str(outp),
        ...
        "seed_bytes": Path(seed).stat().st_size,
        "core_bytes": 0,  # núcleo em python, medido externamente se quiser
        "ratio_seed_vs_orig": (Path(seed).stat().st_size / src.stat().st_size) if src.stat().st_size else 0.0,
        "created": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    }
    (DIR_REP / "prova_p0_ios.json").write_text(json.dumps(rep, ensure_ascii=False, indent=2), encoding="utf-8")
    return rep

# =========================
# Pequeno "CLI" (opcional)
# =========================
def main(argv: List[str] | None = None):
    import argparse
    p = argparse.ArgumentParser(prog="teia-ios", description="TEIA seed-first (Pyto)")
    sub = p.add_subparsers(dest="cmd")

    s_seed = sub.add_parser("seed-anchor"); s_seed.add_argument("file")
    s_idx  = sub.add_parser("index");       s_idx.add_argument("--root", default=str(DIR_INBOX))
    s_loc  = sub.add_parser("locate");      s_loc.add_argument("seed"); s_loc.add_argument("--index", default=str(ANCHOR_INDEX))
    s_res  = sub.add_parser("restore");     s_res.add_argument("seed"); s_res.add_argument("out"); s_res.add_argument("--index", default=str(ANCHOR_INDEX))
    s_cat  = sub.add_parser("catalog")
    s_dry  = sub.add_parser("dryrun")
    s_dash = sub.add_parser("dashboard")
    s_p0   = sub.add_parser("proof-p0");    s_p0.add_argument("--name", default=None)

    args = p.parse_args(argv)

    _ensure_dirs()
    if args.cmd == "seed-anchor":
        sp = seed_anchor(Path(args.file))
        print(sp)
    elif args.cmd == "index":
        ip = build_anchor_index(Path(args.root))
        print(ip)
    elif args.cmd == "locate":
        hits = locate_in_index(Path(args.seed), Path(args.index))
        print(json.dumps(hits, ensure_ascii=False, indent=2))
    elif args.cmd == "restore":
        op = restore_from_index(Path(args.seed), Path(args.out), Path(args.index))
        print(op)
        ...
        print(dashboard())
    elif args.cmd == "proof-p0":
        print(json.dumps(proof_p0(args.name), ensure_ascii=False, indent=2))
    else:
        p.print_help()
...
# 3) Como usar (no Pyto, agora)

1) Crie as pastas e jogue um arquivo de teste (ex.: um vídeo/foto) em `~/Documents/Seeds/inbox/`.  
   (No Pyto: Files → On My iPhone → Pyto → Documents → Seeds → **inbox**.)

2) No Pyto, abra um console Python e rode:

```python
import teia_ios as teia

# Prova P0 completa (seed âncora -> index inbox -> restore -> valida SHA)
teia.proof_p0()                 # usa o primeiro arquivo que achar em inbox/
# ...ou nome explícito (se já estiver dentro de inbox/):
# teia.proof_p0("IMG_1234.MOV")

# Catálogo e dry-run
teia.seed_catalog()
teia.seed_dryrun()

# Gera e abre o dashboard (abre no Safari ou visualizador do Pyto)
teia.dashboard()
```

Saídas principais:
- `reports/prova_p0_ios.json`, `reports/seed_catalog.json`, `reports/dryrun_evict_report.json`
- `dashboard/index.html`
- restaurado em `out/<nome>.anchor.restored.<ext>`

---

# 4) O que já fica “transgredido” no iPhone

- **Seed-first real** (manifest-only): sem varrer iPhone inteiro; apenas `inbox/`.  
- **Index local** (anchors + length) e **restore off-original** (copia do `inbox/` encontrado pelo índice, valida SHA).  
- **Métricas** e **dashboard** local (HTML) como no PC.  
- **Mesma semântica** das rotas do PC: `seed-anchor`, `index`, `locate`, `restore`, `catalog`, `dryrun`, `proof-p0`.

> Ainda não é “procedural que cria o binário do nada” — isso entra com **DICT v0 + CODEC v0** (próximo passo). Mas já deixa **o trilho idêntico** nos dois ambientes.

---

# 5) Próximos 2 passos (quando quiser subir para “procedural real”)

1) **DICT v0 (iOS)**: adicionar `dict/` com **átomos** reutilizáveis (por ex., estruturas estáticas de PNG/MP4, runs, tabelas Huffman canônicas).  
   - O seed passaria a referenciar **IDs de átomos** + parâmetros (em vez de só anchors do original).

2) **CODEC v0 (iOS)**: uma mini-VM reversível (RLE/LZ-like + “emit_atom(id, params)”) para **gerar** bytes no `restore()`.  
   - Primeiro alvo recomendável: **PNG trivial** (IHDR fixo + IDAT com DEFLATE de padrões simples), para provar “|núcleo|+|seed| << |arquivo|” com SHA-match.

Quando isso entrar, o `restore_from_index` vira opcional; o caminho “seed + núcleo ⇒ bytes idênticos” passa a ser local e autônomo.

---

Se topar, eu também te deixo um **presetzinho de lote** (images/docs/áudio/vídeo) para o Pyto depois — mas com esse `teia_ios.py` você já consegue rodar **agora** a Prova P0, ver o **dashboard**, e manter **o mesmo fluxo seed-first** do PC no iPhone.
