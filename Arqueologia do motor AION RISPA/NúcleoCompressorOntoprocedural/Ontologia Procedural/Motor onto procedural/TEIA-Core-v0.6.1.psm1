#Requires -Version 7
# TEIA-Core-v0.6.1.psm1
# Motor de reconstrução procedural — hot-path 100% em C# via [Add-Type]
# Novidades vs v0.6.0:
#   • RestoreInMemory: parse via System.Text.Json + compute + SHA-256 sem nenhum disco I/O
#   • GetOutName: extrai out.name do seed sem ConvertFrom-Json
#   • DecompressStream / DecompressLzmaViaSevZip: descompressão C# pura
#   • Invoke-TEIARestoreRaw: retorna byte[] diretamente (hot-path do benchmark)
#   • Invoke-TEIARestore: modo disco usa RestoreInMemory internamente (elimina ConvertFrom-Json)
# Classe: TEIA.Native.TEIANativeV61 (separada de TEIANative para coexistência no AppDomain)

$script:SEVENZIP_MODULE = "C:\Program Files\7-Zip\7z.exe"

if (-not ('TEIA.Native.TEIANativeV61' -as [type])) {
...
namespace TEIA.Native {
    public static class TEIANativeV61 {

        // ── Primitivas de geração (auto-contidas — não dependem de TEIANative) ────
        ...
        // BuildJsonArray: idêntico ao v0.6.0 — "[\\n" = PS literal backslash+n
        ...
        // BuildJsonArrayFromSchema: constrói byte[] a partir de JsonElement (schema do seed)
        // Usado por RestoreInMemory para evitar re-parse do schema
        ...
        // DecompressStream: descomprime brotli/gzip em memória pura
        ...
        // DecompressLzmaViaSevZip: extrai .7z via processo 7-Zip (sem escrever output final)
        ...
        // GetOutName: extrai out.name do seed JSON via System.Text.Json — sem ConvertFrom-Json
        ...
        // RestoreInMemory: parse via System.Text.Json + compute em C# + SHA-256 — zero I/O de disco
        // Retorna byte[] validado. Para fallback.lzma: requer sevenZipPath não-vazio.
        // Para hot-path sem lzma: passar sevenZipPath = null (lança se encontrar fallback.lzma).
                    ...
                    foreach (var op in root.GetProperty("plan").EnumerateArray()) {
                        ...
                        switch (opCode) {
                            ...
                            case "slice.copy": {
                                ...
                                if (off + sLen > soFar.LongLength)
                                    throw new ArgumentException("slice.copy: janela fora do buffer");
                                byte[] src = new byte[sLen];
                                Buffer.BlockCopy(soFar, (int)off, src, 0, (int)sLen);
                                for (long r = 0; r < rep; r++) ms.Write(src, 0, src.Length);
                                break;
                            }
                            case "fallback.lzma": {
...
function RLE($Pairs) {
    ...
    [TEIA.Native.TEIANativeV61]::DecodeRLE($bVals, $counts)
}

# ── Interfaces públicas ───────────────────────────────────────────────────────

# Invoke-TEIARestoreRaw: hot-path puro — retorna byte[] sem disco I/O
function Invoke-TEIARestoreRaw {
    ...
    # Passa sevenZipPath apenas se houver fallback.lzma; para D1-D4 pode ser vazio.
    [TEIA.Native.TEIANativeV61]::RestoreInMemory($SeedJson, $SevenZipPath)
}

# Invoke-TEIARestore: modo disco — usa RestoreInMemory + WriteAllBytes (backward-compat)
# ConvertFrom-Json eliminado: out.name extraído via GetOutName (C# System.Text.Json)
function Invoke-TEIARestore {
    ...
    $bytes   = [TEIA.Native.TEIANativeV61]::RestoreInMemory($SeedJson, $script:SEVENZIP_MODULE)
    $outName = [TEIA.Native.TEIANativeV61]::GetOutName($SeedJson)
    [IO.Directory]::CreateDirectory((Split-Path -Parent (Join-Path $OutDir $outName))) | Out-Null
    $target  = Join-Path $OutDir $outName
    [IO.File]::WriteAllBytes($target, $bytes)
    $sha = [TEIA.Native.TEIANativeV61]::SHA256Hex($bytes)
