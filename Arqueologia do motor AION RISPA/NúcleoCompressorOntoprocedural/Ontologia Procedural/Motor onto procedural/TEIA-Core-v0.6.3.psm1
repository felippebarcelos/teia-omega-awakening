#Requires -Version 7
# TEIA-Core-v0.6.3.psm1
# Motor de reconstrução procedural — hot-path 100% em C# via [Add-Type]
# Novidades vs v0.6.2:
#   • dict.shared implementado: CompressWithSharedDict / DecompressWithSharedDict via GZip com prefixo de dicionário
#   • BuildSharedDict: constrói dicionário de N arquivos de treinamento (concatenação parcial)
#   • RestoreInMemory aceita dictDir: resolve dict por SHA-256 em diretório pré-distribuído
# Classe: TEIA.Native.TEIANativeV63 (separada de V62 para coexistência no AppDomain)

$script:SEVENZIP_MODULE = "C:\Program Files\7-Zip\7z.exe"

if (-not ('TEIA.Native.TEIANativeV63' -as [type])) {
...
namespace TEIA.Native {
    public static class TEIANativeV63 {
        ...
        // BuildJsonArray: idêntico ao v0.6.2
        ...
        // ── dict.shared — dicionário compartilhado pré-distribuído ────────────

        // Constrói dicionário concatenando parcialmente os arquivos de treinamento.
        // O compressor GZip pode referenciar bytes do dicionário ao comprimir dados similares.
        public static byte[] BuildSharedDict(string[] filePaths, int maxDictBytes) {
            if (maxDictBytes <= 0) throw new ArgumentOutOfRangeException("maxDictBytes", "dict.shared: maxDictBytes deve ser > 0");
            using (var ms = new MemoryStream()) {
                foreach (var path in filePaths) {
                    if (ms.Length >= maxDictBytes) break;
                    if (!File.Exists(path)) continue;
                    byte[] data = File.ReadAllBytes(path);
                    int take = (int)Math.Min(data.Length, maxDictBytes - ms.Length);
                    ms.Write(data, 0, take);
                }
                return ms.ToArray();
            }
        }

        // Comprime data usando dict como prefixo: GZip(dict + data).
        // O compressor vê o dicionário primeiro e pode referenciar seus padrões ao comprimir data.
        // Resultado armazena o fluxo GZip completo — decompressor precisa saber dict_size para separar.
        public static byte[] CompressWithSharedDict(byte[] dict, byte[] data) {
            if (dict == null || dict.Length == 0)
                throw new ArgumentException("dict.shared: dicionário vazio");
            var combined = new byte[dict.Length + data.Length];
            Buffer.BlockCopy(dict, 0, combined, 0, dict.Length);
            Buffer.BlockCopy(data, 0, combined, dict.Length, data.Length);
            using (var outMs = new MemoryStream())
            using (var gz = new GZipStream(outMs, CompressionLevel.Optimal)) {
                gz.Write(combined, 0, combined.Length);
                gz.Close();
                return outMs.ToArray();
            }
        }

        // Descomprime e descarta o prefixo do dicionário (primeiros dictSize bytes).
        // Resultado é apenas os bytes do arquivo original.
        public static byte[] DecompressWithSharedDict(byte[] compressedData, int dictSize) {
            if (dictSize < 0) throw new ArgumentOutOfRangeException("dictSize", "dict.shared: dictSize negativo");
            using (var inMs  = new MemoryStream(compressedData))
            using (var gz    = new GZipStream(inMs, CompressionMode.Decompress))
            using (var outMs = new MemoryStream()) {
                gz.CopyTo(outMs);
                byte[] full = outMs.ToArray();
                if (full.Length < dictSize)
                    throw new InvalidOperationException(
                        "dict.shared: resultado descomprimido (" + full.Length + "B) menor que dicionário (" + dictSize + "B)");
                int fileLen = full.Length - dictSize;
                byte[] result = new byte[fileLen];
                Buffer.BlockCopy(full, dictSize, result, 0, fileLen);
                return result;
            }
        }
        ...
        // ── RestoreInMemory — streaming SHA-256 + dict.shared ────────────────
        // dictDir: diretório contendo arquivos {sha256_lowercase}.dict para dict.shared.
        //          Pode ser vazio se o seed não usar dict.shared.
        public static byte[] RestoreInMemory(string seedJson, string sevenZipPath, string dictDir = "") {
            using (var doc = JsonDocument.Parse(seedJson))
                ...
                using (var ms = new MemoryStream()) {
                    foreach (var op in root.GetProperty("plan").EnumerateArray()) {
                        ...
                        switch (opCode) {
                            ...
                            case "dict.shared": {
                                // Resolve dicionário por SHA-256 no diretório pré-distribuído
                                ...
                                if (string.IsNullOrEmpty(dictDir))
                                    throw new InvalidOperationException(
                                        "dict.shared: dictDir não fornecido — use Invoke-TEIARestoreWithDict -DictDir <path>");

                                string dictFile = Path.Combine(dictDir, dSha.ToLowerInvariant() + ".dict");
                                if (!File.Exists(dictFile))
                                    throw new FileNotFoundException(
                                        "dict.shared: dicionário " + dSha.Substring(0, 16) + "... não encontrado em " + dictDir, dictFile);

                                byte[] dictBytes = File.ReadAllBytes(dictFile);
                                if (dictBytes.Length != dSize)
                                    throw new InvalidOperationException(
                                        "dict.shared: tamanho do dicionário diverge (esperado=" + dSize + " obtido=" + dictBytes.Length + ")");

                                string gotDictSha = SHA256Hex(dictBytes);
                                if (!string.Equals(gotDictSha, dSha, StringComparison.OrdinalIgnoreCase))
                                    throw new InvalidOperationException(
                                        "dict.shared: SHA-256 do dicionário diverge — esperado=" + dSha.Substring(0, 16) + "... obtido=" + gotDictSha.Substring(0, 16) + "...");

                                buf = DecompressWithSharedDict(compressed, dictBytes.Length);
                                sha256.TransformBlock(buf, 0, buf.Length, null, 0);
                                ms.Write(buf, 0, buf.Length);
                                break;
                            }
...
function RLE($Pairs) {
    ...
    [TEIA.Native.TEIANativeV63]::DecodeRLE($bVals, $counts)
}

# ── dict.shared — funções operacionais ───────────────────────────────────────

function Build-TEIASharedDict {
    <#
    .SYNOPSIS
        Constrói dicionário compartilhado a partir de N arquivos de treinamento.
        O dicionário é salvo como {sha256}.dict no diretório de saída.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string[]]$TrainingFiles,
        [Parameter(Mandatory)][string]$OutputDir,
        [int]$MaxDictBytes = 32768
    )
    $valid = @($TrainingFiles | Where-Object { Test-Path $_ })
    if ($valid.Count -eq 0) { throw "Build-TEIASharedDict: nenhum arquivo de treinamento válido" }

    Write-Host "[DICT] Construindo dicionário de $($valid.Count) arquivo(s) (max=$MaxDictBytes B)..."
    $filePaths = [string[]]$valid
    $dictBytes = [TEIA.Native.TEIANativeV63]::BuildSharedDict($filePaths, $MaxDictBytes)
    $dictSha   = [TEIA.Native.TEIANativeV63]::SHA256Hex($dictBytes)

    [IO.Directory]::CreateDirectory($OutputDir) | Out-Null
    $dictPath  = Join-Path $OutputDir ($dictSha + ".dict")
    [IO.File]::WriteAllBytes($dictPath, $dictBytes)

    Write-Host "[DICT] Dicionário: $($dictBytes.Length) bytes | SHA-256: $($dictSha.Substring(0,16))..."
    Write-Host "[DICT] Salvo em: $dictPath"
...
function Invoke-TEIAEncodeWithDict {
    <#
    .SYNOPSIS
        Codifica dados usando dicionário compartilhado pré-construído.
        Retorna objeto com SeedJson, métricas de compressão e SHA-256.
    #>
    [CmdletBinding()]
    param(
    ...
    if (-not (Test-Path $DictPath)) { throw "Invoke-TEIAEncodeWithDict: dicionário não encontrado: $DictPath" }
    ...
    $compressedBytes = [TEIA.Native.TEIANativeV63]::CompressWithSharedDict($dictBytes, $Data)
    $sw.Stop()

    $payloadB64 = [Convert]::ToBase64String($compressedBytes)
    $outSha     = [TEIA.Native.TEIANativeV63]::SHA256Hex($Data)
    ...
    $netRatioPct  = [math]::Round(($seedBytes + $dictSize) / $outSize * 100, 4)
...
function Invoke-TEIARestoreRaw {
    ...
    [TEIA.Native.TEIANativeV63]::RestoreInMemory($SeedJson, $SevenZipPath, $DictDir)
}

function Invoke-TEIARestoreWithDict {
    <#
    .SYNOPSIS
        Restaura arquivo a partir de seed com opcode dict.shared.
        DictDir deve conter {sha256}.dict conforme gerado por Build-TEIASharedDict.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$SeedJson,
    ...
    if (-not (Test-Path $DictDir -PathType Container)) {
        throw "Invoke-TEIARestoreWithDict: DictDir não encontrado: $DictDir"
    }
    [TEIA.Native.TEIANativeV63]::RestoreInMemory($SeedJson, $SevenZipPath, $DictDir)
}
...
function Invoke-TEIARestore {
    ...
    $bytes   = [TEIA.Native.TEIANativeV63]::RestoreInMemory($SeedJson, $script:SEVENZIP_MODULE, $DictDir)
    $outName = [TEIA.Native.TEIANativeV63]::GetOutName($SeedJson)
    [IO.Directory]::CreateDirectory($OutDir) | Out-Null
    $target  = Join-Path $OutDir $outName
    [IO.File]::WriteAllBytes($target, $bytes)
    $sha = [TEIA.Native.TEIANativeV63]::SHA256Hex($bytes)
    return [pscustomobject]@{ Size = $bytes.Length; SHA256 = $sha; Target = $target }
}

Export-ModuleMember -Function 'Invoke-TEIARestore', 'Invoke-TEIARestoreRaw', 'Invoke-TEIARestoreWithDict', 'Build-TEIASharedDict', 'Invoke-TEIAEncodeWithDict'
